...
from engine.utils.exceptions.state_not_found_exception import StateNotFoundException
...
class GameService:
    ...
    def roll_dice(self):
        ...
        try:
            ...
            extra = {}
            extra["error"] = str(e)
            return self.get_state(extra=extra)
    ...
    def place_settlement(self, position: Tuple[float, float]):
        ...
        command = PlaceSettlementCommand(position)
        player = self.game.current_player()
        self.game.execute_command(command, player)
        return self.get_state()
