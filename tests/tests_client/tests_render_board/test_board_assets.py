...
def test_load_tile_icons_caching(monkeypatch):
    ...
    icons = board_assets.load_tile_icons()
    assert isinstance(icons, dict)
    assert all(res in icons for res in ["sheep", "wheat", "brick", "wood", "desert"])
    # Second call uses cache
    icons2 = board_assets.load_tile_icons()
    assert icons2 is icons
